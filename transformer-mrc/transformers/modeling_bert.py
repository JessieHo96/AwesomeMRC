...
class BertForQuestionAnsweringGateMechanism(BertPreTrainedModel):
    r"""
        **start_positions**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`).
...
class BertForQuestionAnsweringGateMechanismAVPool(BertPreTrainedModel):
    r"""
        **start_positions**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`).
    ...
    """
    def __init__(self, config):
        super(BertForQuestionAnsweringGateMechanismAVPool, self).__init__(config)
        self.num_labels = config.num_labels
...
class BertForQuestionAnsweringGateMechanismAVPoolBECV3(BertPreTrainedModel):
    r"""
        **start_positions**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`).
    ...
    """
    def __init__(self, config):
        super(BertForQuestionAnsweringGateMechanismAVPoolBECV3, self).__init__(config)
        self.num_labels = config.num_labels
        ...
        self.has_ans_1 = nn.Sequential(nn.Dropout(p=config.hidden_dropout_prob), nn.Linear(config.hidden_size, 2))
        self.has_ans_2 = nn.Sequential(nn.Dropout(p=config.hidden_dropout_prob), nn.Linear(config.hidden_size, 1))
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        has_log_1 = self.has_ans_1(first_word)
        has_log_1 = has_log.squeeze(-1)
        
        has_log_2 = self.has_ans_2(first_word)
        has_log_2 = has_log.squeeze(-1)
...
#         logits = logits + direct_logits####[batch_size, sequence_length, 2]
        ...
        outputs = (start_logits, end_logits,has_log_1,) + outputs[2:]
        ...
        if start_positions is not None and end_positions is not None:
            ...
            choice_fct = nn.BCEWithLogitsLoss()
            choice_loss1 = loss_fct(has_log_1, is_impossibles)

            
            choice_loss2 = choice_fct(has_log_2, is_impossibles)

            loss_fct = MSELoss()
            choice_loss3 = loss_fct(has_log_2.view(-1), is_impossibles.view(-1))
            
            total_loss = start_loss + end_loss + choice_loss1 + choice_loss2 + choice_loss3
            outputs = (total_loss,) + outputs
...
class BertForQuestionAnsweringLSTM(BertPreTrainedModel):
    r"""
        **start_positions**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`).
    ...
    """
    def __init__(self, config):
        super(BertForQuestionAnsweringLSTM, self).__init__(config)
        self.num_labels = config.num_labels

        self.bert = BertModel(config)
        
        self.lstm = nn.LSTM(config.hidden_size, config.hidden_size, batch_first=True, bidirectional=True, num_layers = 1)
        
        self.highway_net = HighwayNetwork(config.hidden_size)
...
class BertForQuestionAnsweringSeqSCLSTMAVPooler(BertPreTrainedModel):
    def __init__(self, config):
        super(BertForQuestionAnsweringSeqSCLSTMAVPooler, self).__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, pq_end_pos=None, position_ids=None, head_mask=None,
        ...
        logits_1 = self.qa_outputs_1(sequence_output)
        ...
        if start_positions is not None and end_positions is not None:
            ...
            start_loss_1 = loss_fct(start_logits_1, start_positions)
            end_loss_1 = loss_fct(end_logits_1, end_positions)
            
            choice_loss = loss_fct(has_log, is_impossibles)
            total_loss = (start_loss + end_loss + start_loss_1 + end_loss_1 + choice_loss) / 5
