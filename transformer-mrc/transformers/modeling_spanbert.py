...
class SpanBertForQuestionAnswering(BertPreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(SpanBertForQuestionAnswering, self).__init__(config)
        self.bert = BertModel(config)
...
class SpanBertForQuestionAnsweringAVPooler(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    """
    def __init__(self, config):
        super(SpanBertForQuestionAnsweringAV, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, is_impossibles=None):
        ...
        has_log = self.has_ans(cls)

        outputs = (start_logits, end_logits, has_log,)
